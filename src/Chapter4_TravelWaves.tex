...
\begin{document}
    ...
    \textbf{traveling waves}, where $f$ describes the shape of the wave and $|c|$ is the speed of
    the wave.
    ...
    A traveling wave represented by $u(x,y)$ is said to be a \textbf{wave front} if for any fixed t,
    \[
    ...
    If $k_1=k_2$, then the wave front is called a \textbf{pulse}.
    A \textbf{pulse} temporarily changes the value of $u$ at position $x$ before it settles back to
    its original value.
    ...
    \]
    where $A \neq 0$, $k>0$ and $\omega > 0$ are constants is called a \textbf{wave train}.
    ...
    \item $k$ is called the \textbf{wave number} and represents the number of cycles on this period
    ...
    This relationship is called a \textbf{dispersion} relation and indicates
    which values of $k$ and $\omega$ may be selected in order for $u(x, t)$ to be a wave train
    solution.

    A partial differential equation which has wave train solutions is said to be \textbf{dispersive}
